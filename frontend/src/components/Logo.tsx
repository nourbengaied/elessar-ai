...
const Logo: React.FC<LogoProps> = ({ 
  ...
  const renderText = () => (
    <span className="font-inter font-semibold tracking-tight text-lg leading-none text-[#0C1D16] transition-all duration-300 hover:scale-105">
      Dash<span className="bg-gradient-to-r from-[#EC4899] to-[#3B82F6] bg-clip-text text-transparent">board</span>
        </span>
  );
  ...
  if (variant === 'pill') {
    return (
      <div className={`flex items-center gap-1 ${className} group`}>
        <div 
          className="rounded-full bg-gradient-to-r from-[#EC4899] to-[#3B82F6] p-2 shadow-[0_4px_12px_rgba(236,72,153,0.3)] transition-all duration-300 hover:shadow-[0_8px_25px_rgba(236,72,153,0.4)] hover:scale-105"
          style={{ 
            width: `${size + 16}px`, 
            height: `${size + 16}px`,
  ...
  if (variant === 'chip') {
    return (
      <div className={`flex items-center gap-1 ${className} group`}>
        <div 
          className="rounded-full bg-[#FDF2F8] border border-[rgba(236,72,153,0.08)] shadow-[0_1px_2px_rgba(0,0,0,0.04),0_6px_16px_rgba(0,0,0,0.06)] grid place-items-center transition-all duration-300 hover:shadow-[0_4px_20px_rgba(0,0,0,0.1)] hover:scale-105 hover:border-[rgba(236,72,153,0.15)]"
